...
export interface BindAssetElementEventData {
  jobId: string;
  projectId: string;
  assetId: string;
  userId: string;
...
export const bindAssetElementFunction = inngest.createFunction(
  {
    id: "bind-asset-element",
    name: "Bind asset to Kling element",
    triggers: [{ event: "asset/bind_kling_element" }],
    retries: 1,
    concurrency: { limit: 3 },
    onFailure: async ({ event, error }) => {
      const original = (event.data as { event: { data: BindAssetElementEventData } }).event;
      ...
      await supabase
        ...
        .eq("id", original.data.assetId);
    },
  },
  async ({ event, step }) => {
    const data = event.data as BindAssetElementEventData;
    const supabase = createSupabaseServiceClient();
    ...
    const ctx = await step.run("load-context", async () => {
      ...
      if (!asset.confirmed_variation_id) {
        throw new Error("Asset has no confirmed variation. Confirm one first.");
      }

      const { data: variation } = await supabase
        .from("asset_variations")
        .select("image_url")
        .eq("id", asset.confirmed_variation_id)
        .single();
      if (!variation) throw new Error("Confirmed variation not found in storage");

      return {
        name: asset.name,
        description: asset.base_description ?? asset.name,
